...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    """
    result = temperature*neutrons_emitted
    if (temperature<800 and neutrons_emitted>500 and result <500000):
        return True
    else:
        return False
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    lower_bound = threshold - (0.1*threshold)
    upper_bound = threshold + (0.1*threshold)
    low = 0.9 * threshold

    if (result < low):
        return "LOW"
    elif(lower_bound<= result<=upper_bound or result == threshold):
        return "NORMAL"
    else:
        return "DANGER"
